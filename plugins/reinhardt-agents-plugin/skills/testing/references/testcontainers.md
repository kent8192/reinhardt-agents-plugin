...
The default `.sqlite()` backend uses a temporary file. Use
`.sqlite_memory()` for a fast isolated database when the test does not need an
ORM global or DI context. Use `.postgres()` with the `testcontainers` feature
when backend-specific behavior is part of the contract. `.build()` applies the
selected schema before returning the guard.
...
For portable data, load a `FixtureRecord` JSON file after schema creation:
...
The helper uses the active test database and preserves the same transactional
and explicit-primary-key behavior as `manage loaddata`.
