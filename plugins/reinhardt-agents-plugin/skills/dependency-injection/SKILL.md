...
- For 0.4 durable jobs, enable `tasks-durable` and `di`; register one singleton `SharedDurableQueue` with `FactoryOutput<DurableQueueKey, SharedDurableQueue>` and consume it as `Depends<DurableQueueKey, SharedDurableQueue>`
