...
`use_resource` hooks that need SSR timeout or hydration coverage. Identify
hand-composed nested shells that should become `ClientRouter::routes` plus
`#[layout]` / `Outlet`, checking absolute root paths, relative child paths,
index routes, and globally unique route names. Scan `page!` fixtures for
`r#type:` and recommend the direct HTML spelling `type:`.
...
- For Pages SSR/layout migrations, explicitly report synchronous renderer calls, streamed-versus-buffered output assumptions, unresolved native resources, unstable conditional resource keys, and route-tree layout/path/name violations.
