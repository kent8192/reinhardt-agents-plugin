...
| `pages` | "page", "head", "form", "server_fn", "Signal", "useState", "SSR", "hydration", "WASM", "frontend", "ClientLauncher", "ClientRouter", "Outlet", "layout" | WASM frontend with `page!` / `head!` / `form!` macros, reactive hooks, nested layout routes, `ClientLauncher` lifecycle, SPA routing, and SSR |
