...
| `pages` | "page", "head", "form", "server_fn", "Signal", "useState", "SSR", "hydration", "SsrStream", "resource_timeout", "WASM", "frontend", "ClientLauncher", "ClientRouter", "Outlet", "layout" | WASM frontend with `page!` / `head!` / `form!` macros, reactive hooks, nested layout routes, `ClientLauncher` lifecycle, async SSR/resource hydration, SPA routing, and SSR |
...
| `SubagentStart` | (all) | Restores the baseline context for subagents |
| `SessionEnd` | (all) | Removes hook state |

The context hook recognizes an application only when the current directory has
`Cargo.toml`, `src/bin/manage.rs`, and a normal runtime dependency on the
