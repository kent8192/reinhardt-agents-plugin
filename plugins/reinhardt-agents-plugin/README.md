...
| `pages` | "page", "head", "form", "server_fn", "Signal", "useState", "SSR", "hydration", "SsrStream", "resource_timeout", "WASM", "frontend", "ClientLauncher" | WASM frontend with `page!` / `head!` / `form!` macros, reactive hooks, `ClientLauncher` lifecycle, async SSR/resource hydration, SPA routing, and SSR |
