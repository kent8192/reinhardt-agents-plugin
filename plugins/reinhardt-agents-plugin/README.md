...
| `modeling` | "create a model", "add a field", "define relations", "Json<T>", "dumpdata", "loaddata", "seed" | Model definition with 0.4.x app/table identity, typed JSON fields, typed relation traversal, relations (ForeignKey, ManyToMany, OneToOne), `CustomManager`, fixture commands, and migration generation |
...
| `pages` | "page", "head", "form", "server_fn", "Signal", "useState", "use_query", "EventFixture", "style!", "SSR", "hydration", "WASM", "frontend", "ClientLauncher" | WASM frontend with `page!` / `head!` / `form!` macros, typed events, keyed query cache, component-scoped styles, reactive hooks, `ClientLauncher` lifecycle, SPA routing, and SSR |
| `macros` | "#[model]", "#[style_def]", "style!", "#[api]", "#[inject]", "#[admin]", "#[settings]" | Attribute, derive, and function-like procedural macros — semantics, 0.4.x model/DI contracts, style DSL, options, and recommended patterns |
| `testing` | "write tests", "add test coverage", "test this endpoint", "TestDatabase", "dumpdata", "EventFixture", "DI override" | rstest-based test generation with AAA pattern, model-derived `TestDatabase`, fixture data commands, typed Pages event tests, TestContainers, and `with_di_overrides!` DI testing kit |
