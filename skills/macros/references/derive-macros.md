...
Use `Validate` on request/input DTOs. For response DTOs that match a model,
prefer the `{Model}Info` companion generated by `#[model]` instead of creating a
parallel serializer struct with the same fields.
