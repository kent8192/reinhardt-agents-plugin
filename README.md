...
| `pages` | "page", "head", "form", "server_fn", "Signal", "useState", "SSR", "hydration", "WASM", "frontend", "ClientLauncher", "ClientRouter", "Outlet", "SsrStream", "layout" | WASM frontend with `page!` / `head!` / `form!` macros, reactive hooks, nested layout routes, `ClientLauncher` lifecycle, async SSR/resource hydration, SPA routing, and SSR |
